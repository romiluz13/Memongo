...
<!-- scar: 2026-08-01 — a guardrail rejected two `gh issue comment` calls whose bodies happened to contain the literal words "git push" in prose. -->

The git guardrail pattern-matches the literal string `git push` **anywhere in the command**, including inside a `--body` heredoc. Writing it in prose inside an issue body will get the whole `gh` call blocked. Rephrase ("pushed to main", "landed on origin") instead.
