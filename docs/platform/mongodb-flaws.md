...
| Benchmark background work | Raw/session retrieval canaries were blocked by product-grade post-write derivation jobs that belong in dogfood stress lanes, not raw retrieval scoring lanes. | Benchmark/canary agents now default to skipping post-write extraction, entity graph updates, structured/procedure promotion checks, and episode triggers while preserving raw event/chunk projection and lane coverage. `MEMONGO_BENCHMARK_DERIVED_WORK_MODE=enabled` opts dogfood/stress benchmarks back into full derived work. Non-benchmark agents still run full derived work. |
...
| Cross-lane fusion legality | Evidence lived across multiple collections, so `$rankFusion`/`$scoreFusion` could not legally fuse turn/session/preference/temporal lanes as one MongoDB-native surface. | Added an opt-in `memory_evidence` mirror behind `MEMONGO_EVIDENCE_MIRROR_MODE=enabled`, with Search + autoEmbed Vector Search indexes and indexed prefilter fields for `agentId`, `scope`, `scopeRef`, `sessionId`, `unit`, `status`, and `timestamp`. Canonical collections remain source of truth. |
| Preference/session top-5 composition | Preference cases could generate the right candidates but still lose top-5 slots to generic turn hits. | Added evidence-unit provenance, boosted generic `preference`, `userfact`, `session`, and `temporal_anchor` units for matching query intent, and preserved turn provenance for citations. |
| Raw-session index scope | The MemPalace-compatible raw-session lane created and waited on the full product Search/Vector index profile, so unrelated KB/structured/procedure/query-cache indexes could block a raw retrieval smoke. | Added a raw-session Search/Vector profile that creates and checks only the dedicated `session_chunks_vector` autoEmbed index. Full product indexes remain for native and dogfood lanes. |
| Raw-session empty-index wait | Strict bootstrap waited for the `session_chunks_vector` index to be queryable before any benchmark session documents had been written. MongoDB autoEmbed creates generated embeddings lazily after writes, so this could fail a clean run before the lane had evidence to index. | Raw-session bootstrap now defers queryability to the post-ingest convergence gate. Strict mode then probes the actual `$vectorSearch` surface before each scenario is scored. |
| Raw-session autoEmbed readiness | A strict raw-session canary scored 48/48 but one fresh scenario returned an empty top-k because the benchmark waited on text Search visibility while the answering surface was the `session_chunks` autoEmbed Vector Search index. | Raw-session benchmark convergence now probes the exact `$vectorSearch` surface with the `session_chunks_vector` index, `query.text`, and an `agentId` prefilter before evaluation queries run. Fixed sleeps no longer substitute for vector-index readiness in this lane. |
| Managed Atlas resume warm-up | Immediately after unpausing the managed cluster, one strict 48-case run failed with `Error connecting to localhost:28000` from inside Atlas Vector Search. The index was `READY`, but one shard host was absent from `statusDetail`; this was Atlas Search/Vector service warm-up, not a retrieval miss. | Treat this as strict infrastructure failure. Run a tiny `$vectorSearch` autoEmbed probe after cluster resume and require all shard hosts to report `READY/queryable` before using the run as a benchmark gate. Two subsequent fresh strict 48-case runs passed after warm-up. |
...
| Full 500 benchmark claim | Raw-session full-run execution is now unlocked by two consecutive strict 48-case passes, but no public claim exists until a full artifact and MemPalace parity artifact both exist. | Run the full raw-session lane and reproduced MemPalace raw lane from clean shells, then compare only matching retrieval unit, scorer, split, LLM use, and artifact disclosure. |
...
| `raw-session-smoke3-20260520-atlas` | managed Atlas, raw-session lane, derived work disabled, rerank off | `benchmark:status PASS`, 6/6 scored, emptyRate `0`, internal R@5 `1.0000`, session RecallAny@5 `1.0000` | Proves the fresh raw-session profile can boot and score on managed Atlas. Not enough cases to unlock full 500. |
| `raw-session-canary48a-20260520-atlas` | managed Atlas, raw-session lane, derived work disabled, rerank off | `benchmark:status PASS`, 48/48 scored, emptyRate `0`, internal R@5 `0.9861`, session RecallAny@5 `1.0000` | First strict 48 pass after raw-session readiness fixes. |
| `raw-session-canary48b-20260520-atlas` | managed Atlas immediately after cluster resume, raw-session lane, derived work disabled, rerank off | failed strict before scoring with Atlas Vector Search `localhost:28000` connection refused | Infrastructure failure: one shard host was missing from Search index `statusDetail`. Do not count as retrieval quality. |
| `raw-session-canary48c-20260520-atlas` | managed Atlas, raw-session lane, derived work disabled, rerank off | `benchmark:status PASS`, 48/48 scored, emptyRate `0`, internal R@5 `0.9861`, session RecallAny@5 `1.0000` | First successful strict 48 after Atlas Search/Vector warm-up. |
| `raw-session-canary48d-20260520-atlas` | managed Atlas, raw-session lane, derived work disabled, rerank off | `benchmark:status PASS`, 48/48 scored, emptyRate `0`, internal R@5 `0.9861`, session RecallAny@5 `1.0000` | Second consecutive successful strict 48 after warm-up; raw-session full 500 is now technically unlocked. |
