...
import { buildVectorSearchStage, vectorSearch } from "./mongodb-search.js"
import type {
...
export class MongoDBManagerBenchmarkScenarioOps {
	...
	async fastIngestBenchmarkConversations(params: {
		...
		await this.host.insertBenchmarkDocumentsInBatches(
	...
	async waitForBenchmarkSearchConvergence(params: {
		...
		if (params.retrievalLane === "raw-session") {
			await this.host.waitForBenchmarkVectorSearchCollectionConvergence({
		...
		await this.host.waitForBenchmarkSearchCollectionConvergence({
			agentId: params.agentId,
			scope: params.scope,
			scopeRef: params.scopeRef,
			sessionId: params.sessionId,
	...
	async listBenchmarkEventSessions(
		...
		return (await this.host.listBenchmarkEventEvidence(agentId)).sessionIds
	}
	...
	resolveBenchmarkResultSessionIds(
		...
		for (const eventId of this.host.collectBenchmarkResultSourceEventIds(
			result,
		)) {
